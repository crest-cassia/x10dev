...
import tekram.Order;
import tekram.agent.AlterSingleAssetAgent;
import tekram.agent.AlterMarketPriceArbitrageAgent;
import tekram.agent.ArbitrageAgent;
import tekram.agent.MarketPriceArbitrageAgent;
import tekram.agent.CIP2004;
import tekram.agent.CIP2009;
import tekram.agent.CIP2004RegressionAlternative;
import tekram.agent.CIP2009RegressionAlternative;
...
//			if (!isAgentWaitingAnyOrders(agent, markets)) {
				// NOTE: AlternativeAssetAgents submits two orders immediately after closing a circuit breaker.
...
//	}
	...
	public static def isArbitrageTradeable(market:IndexMarket):Boolean {
		val index = market;
...
//					wealth += market.getMarketPrice(t) * agent.getAssetVolume(market);
//				}
//			}
...
//				fundamentalIndex,
	...
	public static def main(args:Rail[String]) {
		if (args.size < 1) {
			throw new Exception("Usage: Main03 config.json");
		}
		val configFile = args(0);
		val json = JSON.parse(new File(configFile));

		val NO_ALTERNATIVE_SINGLE = true;
		val NO_ALTERNATIVE_ARBITRAGE = true;
		...
		Console.OUT.println("# NO_ALTERNATIVE_SINGLE " + NO_ALTERNATIVE_SINGLE);
		Console.OUT.println("# NO_ALTERNATIVE_ARBITRAGE " + NO_ALTERNATIVE_ARBITRAGE);
		...
		for (market in spotMarkets) {
			...
			for (i in 0..(AGENTS_COUNT - 1)) {
				...
				val agent:SingleAssetAgent;
				if (NO_ALTERNATIVE_SINGLE) {
					agent = new SingleAssetAgent(id);
				} else {
					agent = new AlterSingleAssetAgent(id);
				}
				if (AGENT_ORDER_MAKING.equals("CIP2009")) {
					val cip2009:CIP2009;
					if (NO_ALTERNATIVE_SINGLE) {
						cip2009 = new CIP2009(fundamentalWeight, chartWeight, noiseWeight);
					} else {
						cip2009 = new CIP2009RegressionAlternative(fundamentalWeight, chartWeight, noiseWeight);
					}
					...
					val cip2004:CIP2004;
					if (NO_ALTERNATIVE_SINGLE) {
						cip2004 = new CIP2004(fundamentalWeight, chartWeight, noiseWeight);
					} else {
						cip2004 = new CIP2004RegressionAlternative(fundamentalWeight, chartWeight, noiseWeight);
					}
				...
				if (!NO_ALTERNATIVE_SINGLE) {
					for (m in spotMarkets) {
						if (m != market) {
							(agent as AlterSingleAssetAgent).setSecondaryMarketAccessible(m);
						}
					}
				}
		...
		for (market in indexMarkets) {
			...
			for (i in 0..(AGENTS_COUNT - 1)) {
				val id = agents.size();
				val agent:MarketPriceArbitrageAgent;
				if (NO_ALTERNATIVE_ARBITRAGE) {
					agent = new MarketPriceArbitrageAgent(id);
				} else {
					agent = new AlterMarketPriceArbitrageAgent(id);
				}
				agent.orderMinVolume = AGENT_ORDER_MIN_VOLUME;
				agent.orderThresholdPrice = AGENT_ORDER_THRESHOLD_PRICE;
...
//			assert indexMarkets.size() == 1;
//			if (!NO_ALTERNATIVE_ARBITRAGE || isArbitrageTradeable(indexMarkets(0))) {
...
//			}
		...
		for (t in 1..MARKET_ITERATION) {
			...
			if (false) {
				markets(1).buyOrderBook.dump();
				markets(1).sellOrderBook.dump();
			}
